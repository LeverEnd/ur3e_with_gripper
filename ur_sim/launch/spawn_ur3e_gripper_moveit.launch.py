...
def generate_launch_description():
    ...
    joint_controllers_file = os.path.join(
        get_package_share_directory('ur_sim'), 'config', 'ur3e_gripper_controllers.yaml'
    )
    ...
    world_file = os.path.join(
        get_package_share_directory('ur_sim'), 'worlds', 'cubes.sdf')
    ...
    gazebo = IncludeLaunchDescription(
        ...
        launch_arguments={
            "gz_args": f" -r -v 4 {world_file}"}.items(),
        #launch_arguments={
        #    'use_sim_time': 'true',
        #    'debug': 'false',
        #    'gui': 'true',
    ...
    rviz_config_path = os.path.join(
        get_package_share_directory("ur_gripper_moveit"),
        "config",
        "moveit.rviz",
    )
    ...
    delay_arm_trajectory_controller_spawner = RegisterEventHandler(
        #OnProcessStart(
    ...
    ld.add_action(delay_arm_trajectory_controller_spawner)
    ld.add_action(delay_gripper_controller_spawner)
